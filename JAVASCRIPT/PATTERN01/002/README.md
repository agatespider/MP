...
## 1. Jasmine 예제
Jasmine은 공식 홈페이지에 가서 보면 Javascript를 테스트 하기 위한 Behaivor-driven develoment Framework라고 설명이 되어 있습니다.

간단히 말하면 Javascript로직을 테스트 하기 위한 도구라고 볼 수 있습니다.

TestFrameworks_01.js안의 createReservation는 passenger와 flight를 인자로 받아서 그 2개의 인자를 속성으로 갖는 객체를 리턴하며 TestFrameworks_01_tests.js는 createReservation함수를 테스트 하기 위한 코드입니다.
  
Jasmine을 사용해서 테스트 하기 위해선 문자열과 함수를 인자로 갖는 전역 describe라는 함수를 호출하면서 시작합니다.
...
첫번째 인자는 문자열이며 어떤 테스트 할지에 대한 주석이며 두번째 파라메터 함수는 실제 로직을 테스트하기 위한 구역입니다. 전역 함수 it을 통해서 테스트 코드를 작성 할 수 있습니다.
    
테스트는 값을 인자로 갖는 expect함수로 테스트를 시작 합니다. 각각의 expect는 인자의 값과 예상되는 값을 bool비교를 합니다.

실제 bool비교를 하는 것은 matcher가 담당합니다. 위의 코드에서 matcher는 tobe함수이며 비교를 하고 true, false를 반환 하는 역할을 합니다.

예제는 tobe matcher를 사용해서 실제 값과 가상 값이 같은지 테스트를 하고 있습니다. 하지만 같지 않을 경우도 테스트 할 수 있어야 하기 때문에 not을 통해서 부정에 대한 테스트도 할 수 있습니다.
...
기존 예제에서 예약저장을 위해 서버에 데이터를 저장하는 기능을 담당하는 ReservationSaver라는 서비스가 새로 생겻고 createReservation은 이것을 이용해서 서버에 예약정보를 저장해야 한다고 생각을 해봅시다.

자 그럼 createReservation입장에서의 테스트 코드는 ReserationSaver를 인자로 받고 ReservationSaver의 saveReservation함수를 호출하는 것을 테스트 해야합니다.

어떻게 테스트를 해야할까요? jasmine의 spy는 위같은 상황을 테스트 할 수 있도록 도와 줍니다..
...
spy는 위처럼 사용 할 수 있으며 호출을 한 시점 정보 및 여러가지 History성 정보를 가지고 있습니다.
 
     
## 2. 왜 테스트를 하는가?
테스트 코드를 작성시 테스트 대상 코드를 먼저 개발하고 테스트 코드를 만드는 것이 아닙니다.
     
실제 구현 코드를 먼저 구현하기 전 테스트 코드를 작성을 해야합니다. 이유는 코드를 개발함에 있어서 한번더 생각을 하게 해줍니다. 생각을 하게 해준다는 것은 내가 지금 개발할 기능에 대해서 주요 관심사로서 설계를 하게 된다는 뜻이 됩니다.
     
또한 코드의 테스트 용이성과 그 테스트가 얼마나 잘 이루어졌는지에 대해서 코드가 유지보수성이나 확장성이 우수한지 우수하지 않은지 알 수도 있습니다.
    
예를 들어 createReservation는 예약을 생성하는 기능을 추가하는데 추기된 기능에서 예약을 등록하기 위해 ajax로 서버에 통신을 해야한다고 생각해봅시다.

그냥 jQuery.ajax() 호출 코드를 createReservation에 추가해서 동작시키면 간단하게 처리 할 수 있습니다. 하지만 createReservation안에서 여러 기능이 추가 되면서 createReservation의 역할이 늘었고 단위 테스트 하기가 어렵게 되어버렸습니다.

귀찮으니까 테스트 코드보다 그냥 동작하니 위 처럼 코드를 추가하고 진행 하고 싶을 것입니다. 하지만 그렇게 하는것은 차후에 도움이 되질 않습니다.
